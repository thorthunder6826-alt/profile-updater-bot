...
- **Parallel processing**: Profile renames run sequentially with single browser reuse (~6s per profile, ~30s for 5 profiles) with 3 retries each. PINs run sequentially (~7s per profile, ~35s for 5 profiles) using single browser reuse.
