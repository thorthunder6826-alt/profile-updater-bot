...
- **Parallel processing**: Profile renames run in parallel (3 concurrent pages in single browser, ~4s per profile, ~10-15s for 5 profiles) with 3 retries each. PINs run sequentially (~7s per profile, ~35s for 5 profiles) using single browser reuse.
...
- **Profile rename MFA**: Profile name edits on locked profiles may trigger MFA - the bot handles password dialogs and retries up to 3 times per profile. No delete+recreate fallback is used (preserves PINs, icons, GUIDs).
